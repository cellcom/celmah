...
public static class ErrorXml
{
    ...
    private static void WriteParamsLog(XmlWriter writer, string name, IEnumerable<CelmahLogParamEntry> log)
    {
        if (log == null)
        {
            return;
    ...
    private static void WriteSqlLog(XmlWriter writer, string name, IEnumerable<CelmahLogSqlEntry> log)
    {
        if (log == null)
        {
            return;
    ...
    private static void WriteMessageLog(XmlWriter writer, string name, IEnumerable<ICelmahLogMessage> log)
    {
        if (log == null)
        {
            return;
    ...
    private static void UpcodeToLog(XmlReader reader, ICollection<ICelmahLogMessage> log)
    {
        ArgumentNullException.ThrowIfNull(reader);
    ...
    private static void UpcodeToParams(XmlReader reader, ICollection<CelmahLogParamEntry> log)
    {
        ArgumentNullException.ThrowIfNull(reader);
        ArgumentNullException.ThrowIfNull(log);
        ...
        while (reader.NodeType != XmlNodeType.EndElement)
        {
            if (reader.IsStartElement("parameter"))
            {
                ...
                if (list.Any())
                {
                    log.Add(new CelmahLogParamEntry(
                        timeStamp,
                        list.ToArray(),
                        typeName,
                        memberName!,
    ...
    private static void UpcodeToSqlLog(XmlReader reader, ICollection<CelmahLogSqlEntry> log)
    {
        ArgumentNullException.ThrowIfNull(reader);
        ArgumentNullException.ThrowIfNull(log);
        ...
        while (reader.NodeType != XmlNodeType.EndElement)
        {
            ...
            {
                var entry = new CelmahLogSqlEntry
                {
