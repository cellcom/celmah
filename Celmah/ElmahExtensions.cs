...
public static class CelmahExtensions
{
    public static Task RaiseErrorAsync(this HttpContext context, Exception exception)
    {
        var logger = context.RequestServices.GetRequiredService<ICelmahExceptionLogger>();
    ...
    {
        var feature = ctx.Features.Get<ICelmahLogFeature>();
        if (feature is null)
        {
            return;
        }
