...
internal sealed class CelmahSqlDiagnosticObserver : IObserver<DiagnosticListener>, IDisposable
{
    private readonly IHttpContextAccessor _httpContextAccessor;
    private readonly IOptions<CelmahOptions> _options;
    private readonly List<IDisposable> _subscriptions = new();

    public CelmahSqlDiagnosticObserver(IHttpContextAccessor httpContextAccessor, IOptions<CelmahOptions> options)
    {
    ...
    private ICelmahLogFeature? GetCelmahContext()
    {
        try
        {
            return _httpContextAccessor.HttpContext?.Features.Get<ICelmahLogFeature>();
        }
    ...
    private void OnCommandStart(Guid id, DbCommand cmd)
    {
        ...
        if (query.Contains("/* celmah */"))
        {
            return;
        }

        var context = this.GetCelmahContext();
        if (context is null)
        {
            return;
        }
        ...
        context.AddSql(id, new CelmahLogSqlEntry
        {
