...
public sealed class Error : ICloneable
{
    ...
    private List<ICelmahLogMessage>? _messageLog;
    private List<CelmahLogSqlEntry>? _sqlLog;
    private List<CelmahLogParamEntry>? _params;
    ...
    [AllowNull]
    public ICollection<ICelmahLogMessage> MessageLog
    {
        get => _messageLog ??= new List<ICelmahLogMessage>();
        init => _messageLog = value?.ToList();
    }

    [AllowNull]
    public ICollection<CelmahLogSqlEntry> SqlLog
    {
        get => _sqlLog ??= new List<CelmahLogSqlEntry>();
        set => _sqlLog = value?.ToList();
    }

    [AllowNull]
    public ICollection<CelmahLogParamEntry> Params
    {
        get => _params ??= new List<CelmahLogParamEntry>();
